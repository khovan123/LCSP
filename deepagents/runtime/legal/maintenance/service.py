"""Proactive legal source refresh and deterministic corpus recovery.

The model cannot supply arbitrary URLs. This service discovers only already-approved
source manifests in the LCSP corpus store, refreshes those sources, and hands any
changed artifacts to the existing governed recovery/activation pipeline.
"""
...
from runtime.legal.sources.recovery.legal_corpus_recovery_driver import LegalCorpusRecoveryDriver
from runtime.legal.sources.scripts.crawl_vbpl_document import VbplDocumentCrawler


class LegalIntelligenceMaintenanceService:
    """Refresh approved legal sources and recover only when content changes."""
    ...
    def run(self, *, max_runs: int = 500, correlation_id: str | None = None) -> dict[str, Any]:
        """Crawl approved manifests, recover changed corpus, and expose affected scope."""
        manifests = sorted(
            path
