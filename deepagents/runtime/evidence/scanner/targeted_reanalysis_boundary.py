...
from tools.common.managed.boundary import (
    AgentBoundaryBase,
...
TARGETED_REANALYSIS_BOUNDARY_SOURCE = "scan.targeted-reanalysis"
...
class TargetedReanalysisBoundary(AgentBoundaryBase):
    """Consumes only immutable, API-authorized targeted-reanalysis commands.

    The invocation payload is treated as an untrusted delivery hint. Before running any
    analyzer, the boundary reloads the authoritative request from the API and
    requires the event identifiers and normalized scope to match exactly. This
    prevents a stale or forged invocation message from widening the approved scan scope.
    """
    ...
    def __init__(
        ...
        """Initialize the boundary and its API/scan dependencies.
        ...
        self._scan_runner = scan_runner or ScanBoundary(
            config,
    ...
    def handle(self, message: dict, correlationId: str) -> None:
        ...
        Raises:
            NonRetryableAgentBoundaryError: For invalid scope, privacy failures, event/API
        ...
        try:
            ...
            raise NonRetryableAgentBoundaryError("targeted reanalysis privacy assertion failed") from error
    ...
    def _handle_execution_failure(
        ...
        Raises:
            NonRetryableAgentBoundaryError: Once the configured retry budget is exhausted.
        ...
        if envelope.delivery_attempt >= self._config.max_retries:
            ...
            raise NonRetryableAgentBoundaryError("targeted reanalysis retries exhausted") from error
    ...
    def _read_envelope(
        ...
        """Validate invocation payload shape and normalize aliases into one envelope.

        Args:
            message: Raw invocation message.
        ...
        Raises:
            NonRetryableAgentBoundaryError: If required identifiers, scope, or delivery-attempt
        ...
        if (
            ...
            raise NonRetryableAgentBoundaryError("targeted reanalysis event is invalid")

        delivery_attempt = message.get("_delivery_attempt", 0)
        if not isinstance(delivery_attempt, int) or delivery_attempt < 0:
            raise NonRetryableAgentBoundaryError("targeted reanalysis delivery attempt is invalid")
    ...
    def _assert_matches_authorized_request(
        ...
        """Require the invocation envelope to equal the API-authorized immutable request.

        Args:
            envelope: Values received from the Managed Agent invocation.
            request: Authoritative request loaded from the API.

        Raises:
            NonRetryableAgentBoundaryError: If any identity, checkpoint, commit, analyzer,
        ...
        if any(request.get(key) != value for key, value in expected.items()) or (
            ...
            raise NonRetryableAgentBoundaryError("targeted reanalysis event did not match request")
    ...
    def _canonical_scope(value: object) -> str | None:
        ...
        if not TargetedReanalysisBoundary._is_scope(value):
            return None
