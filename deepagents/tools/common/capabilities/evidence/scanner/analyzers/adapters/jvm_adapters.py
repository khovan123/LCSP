...
class _JvmLanguageAdapter:
    ...
    def analyze(self, workspace: Path, include_files: Sequence[str] | None) -> CanonicalAnalyzerResult:
        native = JvmAnalyzer(workspace, self.language).analyze(include_files)
...
class JavaLanguageAdapter(_JvmLanguageAdapter):
    language = "java"
    extensions = (".java",)


class KotlinLanguageAdapter(_JvmLanguageAdapter):
    language = "kotlin"
    extensions = (".kt",)
