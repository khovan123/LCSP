...
def validate_claim_topology(
    *,
    ...
    if kind == "HUMAN_CONTROL_STATE":
        _validate_human_control(selected_edges, selected_nodes)
        return
...
def _criterion_kind(criterion: str | None) -> str | None:
    tokens = {
        token.lower()
...
def _validate_downstream_action(edges: list[dict], nodes: dict[str, dict]) -> None:
    ...
    effect_edges = [
        ...
        and str(edge.get("source_node_id") or "") in reached_actions.union(actions)
    ]
    if effect_edges:
        return
...
def _validate_human_control(edges: list[dict], nodes: dict[str, dict]) -> None:
    for edge in edges:
        ...
        if (
            source_type in _ACTION_TYPES
            and target_type in _HUMAN_TYPES
            and _trusted(source)
            and _trusted(target)
        ):
            return
