...
class JavaScriptArchitectureResolver:
    ...
    def _kafka_subscriptions(sources: Iterable[_Source]) -> dict[tuple[str, str], set[str]]:
        ...
        for source in sources:
            ...
            for match in _KAFKA_RUN_RE.finditer(source.text):
                consumer, handler = match.group(1), match.group(2)
                for topic in topics.get(consumer, []):
                    result.setdefault((consumer, topic), set()).add(handler)
        return result
    ...
    def _brokers(
        ...
        for match in _RABBIT_CONSUME_RE.finditer(source.text):
            ...
            _consumer(program, symbols, source, "RABBITMQ", queue, handler, match.start())
        ...
        for match in _BULL_WORKER_RE.finditer(source.text):
            ...
            _consumer(program, symbols, source, "BULLMQ", queue, handler, match.start())
        ...
        for (consumer, topic), handlers in kafka_topics.items():
            if not any(m.group(1) == consumer and m.group(2) == topic for m in _KAFKA_SUB_RE.finditer(source.text)):
                continue
            for handler in handlers:
                _consumer(program, symbols, source, "KAFKAJS", topic, handler, 0)
