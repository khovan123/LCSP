...
from lcsp_workers.scanner.inventory.language_types import LANGUAGE_PYTHON
...
def _provenance(name: str, outcome: str = OUTCOME_SUCCESS) -> ToolProvenance:
    return ToolProvenance(
        ...
        language_profile=RepositoryLanguageProfile(languages=(LANGUAGE_PYTHON,), file_counts={LANGUAGE_PYTHON: 1}),
        disposition="ACCEPTED" if outcome == OUTCOME_SUCCESS else "RETRYABLE_FAILURE",
        retryable=outcome != OUTCOME_SUCCESS,
        evidence_eligible=outcome == OUTCOME_SUCCESS,
        blocking_reason=None,
        coverage_limitations=[] if outcome == OUTCOME_SUCCESS else ["tool failed in fixture"],
        context={},
    )
...
def _python_analysis() -> PythonAnalysisResult:
    return PythonAnalysisResult(
        files_analyzed=1,
        files_skipped=0,
        ai_call_sites=[AiCallSite(file_path="src/app.py", line_number=8, function_name="client.responses.create", module_alias="openai", matched_rule_id="openai.responses.create", finding_type="AI_MODEL_INVOCATION", analysis_level="L1", call_args_schema=["input"], has_dynamic_call=False, kwarg_names=["input"], confidence=0.95, evidence=[{"file": "src/app.py", "line": 8}])],
...
def _assemble(**overrides):
    values = dict(
        ...
        syft_result=_syft(),
        semgrep_result=SemgrepRunResult(findings=[], executions=[_execution("semgrep")], redaction_applied=True),
        coverage_notes=[],
        package_dependencies=[PackageDependency(name="openai", version="1.0.0", ecosystem="python", purl="pkg:pypi/openai@1.0.0", usage_facts=[], confidence_boost=0.0, is_ai_relevant=True, license_expression="MIT")],
        dependency_executions=[_execution("knip"), _execution("deptry")],
        python_analysis=_python_analysis(),
        ts_js_analysis=_ts_analysis(),
        technical_findings=[],
        structural_facts=[StructuralFact(file_path="src/app.py", graph_node_type="FUNCTION", name="generate", line_number=5, pattern_type="function", ai_finding_ids=[])],
        evidence_graph=_graph(),
        scan_coverage=[SimpleNamespace(file_path="src/app.py", language="python", support_level="FIRST_CLASS", reason="supported")],
        targeted_reanalysis=None,
        tool_provenance=[_provenance("syft"), _provenance("semgrep"), _provenance("knip"), _provenance("deptry"), _provenance("ts-morph"), _provenance("python-ast"), _provenance("libcst"), _provenance("tree-sitter")],
    )
...
def test_t02_failed_tool_keeps_explicit_coverage_limit() -> None:
    failed = _execution("syft", OUTCOME_TOOL_FAILURE)
    report = _assemble(syft_result=SyftRunResult(entries=[], execution=failed), coverage_notes=["syft failed in fixture"], tool_provenance=[_provenance("syft", OUTCOME_TOOL_FAILURE), _provenance("semgrep"), _provenance("knip"), _provenance("deptry"), _provenance("ts-morph"), _provenance("python-ast"), _provenance("libcst"), _provenance("tree-sitter")])
    assert report.evidence_payload["coverageLimitations"]


def test_t03_source_code_flag_fails_closed() -> None:
    assembler = EvidenceAssembler(); payload = _assemble().evidence_payload; payload["containsSourceCode"] = True
...
def test_t07_dependency_tool_executions_are_serialized() -> None:
    ...
    assert {row["tool_name"] for row in report.evidence_payload["dependencyToolExecutions"]} == {"knip", "deptry"}


def test_t08_structural_facts_are_sanitized() -> None:
    facts = _assemble().evidence_payload["structuralFacts"]
    assert facts and facts[0]["file_path"] == "src/app.py" and "source" not in facts[0]


def test_t09_serializes_sanitized_versioned_program_evidence_graph() -> None:
    graph_payload = _assemble(evidence_graph=_graph()).evidence_payload["evidence_graph"]
    ...
    assert "evidence:finding-1" in next(node for node in graph_payload["nodes"] if node["node_type"] == "FILE")["evidence_refs"]
    serialized = str(graph_payload).lower()
    assert "raw_source" not in serialized and "full_ast" not in serialized and "prompt_text" not in serialized
...
def test_t11_tool_provenance_is_complete_and_sanitized() -> None:
    rows = _assemble().evidence_payload["toolProvenance"]
    assert rows and all(row["tool_name"] for row in rows) and all(row["config_hash"] for row in rows)
