...
from lcsp_workers.investigation.initial_query_executor import InitialQueryExecutor
from lcsp_workers.legal.engineering_rules.models import EngineeringRule, GraphQueryTemplate
...
def test_seed_query_records_candidate_search_once_and_defers_path_traversal() -> None:
    engine = _Engine()
    with patch(
        "lcsp_workers.investigation.initial_query_executor.ProgramGraphQueryEngine",
        return_value=engine,
    ):
