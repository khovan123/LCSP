...
from lcsp_workers.scanner.program_graph.framework_resolution import (
...
def test_queue_consumer_continues_to_process_method_and_di_target(tmp_path) -> None:
    ...
    program = FrameworkBoundaryResolver(tmp_path).enrich(SemanticProgram())
    edges = _edge_set(program)
    handler = "framework-method:consumer.ts:EngineeringConsumer.handle"
    worker = "framework-method:consumer.ts:WorkerService.run"
    worker_call = next(
        node.key
        for node in program.nodes
        if node.node_type == "CALL_SITE" and node.label == "this.worker.run"
...
def test_event_consumer_is_bound_to_concrete_method(tmp_path) -> None:
    source = tmp_path / "event-consumer.ts"
    source.write_text(
        """
    ...
    assert (
        ...
        "framework-method:event-consumer.ts:AuditConsumer.handleCompleted",
    ) in _edge_set(program)
