...
|---|---|---|---|---|---|---|
| `event.scan.completed.v1` | Scanner Worker | Technical Profile Worker / orchestrator | Repository scan completes and `TechnicalEvidenceReport` is persisted. | `assessmentId`, `scanJobId`, `technicalEvidenceReportId`, `reportHash` | `scanJobId + reportHash` | If consumer fails, retry via queue; source report remains immutable. |
...
| `event.classification.completed.v1` | Classification Worker | Document Worker / reporting projection | ClassificationResult is persisted as completed. | `assessmentId`, `classificationResultId`, `verifiedProfileId`, `citationCoverage` | `classificationResultId` | Retry downstream document command. |
| `event.classification.blocked.v1` | Classification Worker | Manager UI projection / Document guardrail | Classification cannot complete. | `assessmentId`, `classificationResultId`, `blockingReasons` | `assessmentId + verifiedProfileId + blockingReasonHash` | Keep report blocked; show reason. |
...
| `CLASSIFICATION_COMPLETED` | Classification Worker | Document Worker / Audit | ClassificationResult completes. | `assessmentId`, `classificationResultId` | `classificationResultId` | Retry document command if needed. |
| `CLASSIFICATION_BLOCKED` | Classification Worker | Manager UI / Audit | Classification cannot complete. | `assessmentId`, `classificationResultId`, `blockingReasons` | `classificationResultId + blockingReasonHash` | Surface blocking reason. |
