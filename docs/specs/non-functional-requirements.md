...
| NFR-017 | Traceability | Legal classification outputs must trace to legal rule, EngineeringRule, citation and corpus version. | 100% critical conclusions include rule/citation/version refs. | EngineeringRule/classification contract tests. | EngineeringRule Assessment, Classification | Must |
| NFR-018 | Compliance Support | System must fail closed for missing critical evidence, unresolved conflict, unknown critical usage or missing legal citation. | Classification/report blocked or degraded with reason. | State-machine and worker tests. | AIUsageFlow, EngineeringRule Assessment, Classification, Document | Must |
| NFR-019 | Compliance Support | Classification must use evidence-backed EngineeringRule evaluations, not provider/model/framework presence alone. | Provider-only fixture never yields final risk classification. | Rule tests. | AIUsageFlow, EngineeringRule Assessment, Classification | Must |
...
| NFR-021 | Reliability | Long-running scan, EngineeringRule assessment, classification and document work must not depend on web request lifecycle. | Job status and failure reason persist after request completion. | Queue/job tests. | Scanner, EngineeringRule Assessment, Classification, Document | Must |
...
| NFR-024 | Scalability | API runtime and Python Worker Platform workloads must remain separable. | Scan/profile/AIUsageFlow/EngineeringRule/classification/gap/document/export work executes asynchronously in bounded Python consumers/modules. | Architecture/code review and worker tests. | Backend, Python Workers | Should |
...
| NFR-029 | Traceability | AIUsageFlow claims must carry evidence refs and uncertainty reasons for material fields. | Material claim without evidence ref is not EngineeringRule/classification eligible. | AIUsageFlow rule tests. | AIUsageFlow, EngineeringRule Assessment | Must |
| NFR-030 | Reliability | Re-runs must preserve historical evidence/profile/classification chain rather than mutating prior records. | New run creates new versioned objects; prior chain remains traceable. | Persistence and audit tests. | Scanner, Technical Profile, AIUsageFlow, EngineeringRule Assessment | Must |
...
| NFR-034 | Compliance Support | Pinned legal corpus snapshots (`LegalCorpusVersion`) must remain immutable, with updates governed by a formal review and approval process. | Approved corpus versions only are usable for EngineeringRule classification. | Integration and retriever test suite. | EngineeringRule Assessment, Classification | Must |
