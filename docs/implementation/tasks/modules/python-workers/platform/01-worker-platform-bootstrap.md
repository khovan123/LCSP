...
Bootstrap the LCSP Python Worker Platform: RabbitMQ consumer base class, structured logging, RBAC preflight client, correlation ID propagation, and graceful shutdown. All domain workers inherit from this platform. No domain logic in platform layer.
...
| `deepagents/tools/common/platform/consumer_base.py`  | Create | Base RabbitMQ consumer with retry/DLQ and RBAC preflight |
| `deepagents/tools/common/platform/logging_config.py` | Create | Structured JSON logging — no secrets in output           |
| `deepagents/tools/common/platform/rbac_client.py`    | Create | HTTP client for `/internal/rbac/preflight`               |
...
class ConsumerBase:
    ...
    requires_rbac: bool = True              # Override to False for system-only workers
...
3. If `requires_rbac = True`: call RBAC preflight. If deny → nack and discard. If unreachable → nack and retry.
...
3. RBAC preflight called before any user-context task processing.
...
| --- | ---------------------------- | ---------------------------------------------------------------- |
| T01 | Valid message + RBAC allow   | `handle()` called                                                |
| T02 | RBAC deny                    | Message discarded (nack no-requeue), `WORKER_TASK_DENIED` logged |
| T03 | RBAC preflight unreachable   | Message nacked + retry                                           |
...
- RBAC preflight runs before every user-context task.
