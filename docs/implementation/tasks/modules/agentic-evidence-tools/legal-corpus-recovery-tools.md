...
|---|---|---|---|
| `get_admin_source_catalog` | document identity or catalog ID → catalog source/document ref and allowed host/path policy | Resolve only admin catalog; no URL field. System/orchestrator only; LLM sees identity/ref. | Unknown/wrong identity/URL param denied. Catalog repo/contracts; test SSRF boundary/PBAC. |
