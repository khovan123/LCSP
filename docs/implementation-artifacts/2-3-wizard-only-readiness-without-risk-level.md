...
- [ ] Project readiness-only overview when WizardProfile exists but technical evidence does not. (AC: 1)
- [ ] Lock classification with neutral status and missing-evidence checklist. (AC: 1)
- [ ] Project explicit unknown-state and unresolved business/legal context in readiness messaging from `WIZARD-MAPPING.md` without implying final legal consequence. (AC: 1)
- [ ] Audit transition when evidence later becomes available without mutating original WizardProfile version. (AC: 2)
...
- Source packet: `docs/developer/story-handbook/2-3-wizard-only-readiness-without-risk-level.md`.
...
- docs/implementation-artifacts/2-3-wizard-only-readiness-without-risk-level.md
