# DEV-BLUEPRINT — STORY-05-01 — AIUsageFlow Claim Model

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **AIUsageFlow Claim Model** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/ai-usage-flow/aiusageflow-claim-model.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/ai-usage-flow/aiusageflow-claim-model.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/ai-usage-flow/dto/aiusageflow-claim-model.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/ai-usage-flow/repositories/aiusageflow-claim-model.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/ai_usage_flow/aiusageflow_claim_model-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/ai-usage-flow/aiusageflow-claim-model.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/ai-usage-flow/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/ai-usage-flow/aiusageflow-claim-model.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/ai-usage-flow` or story-specific nested route.
- Form fields: IDs/references required for TechnicalProfile and evidence graph; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:05-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:05-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show AIUsageFlow claims with evidence refs.
...
3. Load predecessor state required by `STORY-04-04` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-04-04` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements AIUsageFlow Claim Model | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-04-04`.
...
This story turns `STORY-05-01` into an executable controlled-prototype slice. The business outcome is: AIUsageFlow claims with evidence refs. It gives the developer exact ownership for package `@lcsp/ai-usage-flow`, API route `/api/v1/assessments/{assessmentId}/ai-usage-flow`, service `AIUsageFlowService`, and persistence models `AIUsageFlow, AIUsageFlowClaim, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-04-04.
...
1. Create shared contract in `packages/contracts/src/story-05-01/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: AIUsageFlow claims with evidence refs.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `TechnicalProfile and evidence graph`.
...
→ output state: API/UI exposes AIUsageFlow claims with evidence refs; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: AIUsageFlow claims with evidence refs.
