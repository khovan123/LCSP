# DEV-BLUEPRINT — STORY-08-03 — Document Generation Workflow Shell

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Document Generation Workflow Shell** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/classification` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/classification` | Manager-facing action, loading/error/blocked state |
| API | `classification / gap-analysis / document` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.classification` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | ClassificationRun, RiskClassificationResult, GapAnalysisResult, ComplianceDocument, GeneratedDocumentFile, AuditEvent |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/classification/document-generation-workflow-shell.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/classification/document-generation-workflow-shell.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/classification/dto/document-generation-workflow-shell.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/classification/repositories/document-generation-workflow-shell.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/classification/document_generation_workflow_shell-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/classification/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend ClassificationRun, RiskClassificationResult, GapAnalysisResult, ComplianceDocument, GeneratedDocumentFile, AuditEvent models |
| CREATE | `packages/contracts/src/classification/document-generation-workflow-shell.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/classification/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/classification/document-generation-workflow-shell.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/classification` or story-specific nested route.
- Form fields: IDs/references required for VerifiedProfile, classification, gap analysis, citations; no raw source, secrets or full prompts.
...
- Route: `/api/v1/documents/{assessmentId}/generate`.
...
```json
{
  ...
  "inputReference": "ref:08-03-prototype",
...
- Routing key: `command.document.requested.v1` when async work is required.
- Job payload JSON:
```json
{
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:08-03-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: ClassificationRun, RiskClassificationResult, GapAnalysisResult, ComplianceDocument, GeneratedDocumentFile, AuditEvent.
...
### UI Output
- Success state: show ComplianceDocument metadata and GeneratedDocumentFile ref.
...
Use Prisma migrations to add or extend ClassificationRun, RiskClassificationResult, GapAnalysisResult, ComplianceDocument, GeneratedDocumentFile, AuditEvent. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-08-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-08-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
- Provider/model/framework detection alone is not legal-risk proof; legal conclusions require citation traceability.
...
|---|---|---|
| Implements Document Generation Workflow Shell | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-08-02`.
- Intended wave: Wave 5.
...
This story turns `STORY-08-03` into an executable controlled-prototype slice. The business outcome is: ComplianceDocument metadata and GeneratedDocumentFile ref. It gives the developer exact ownership for package `@langchain/langgraph`, API route `/api/v1/assessments/{assessmentId}/documents`, service `DocumentService`, and persistence models `GeneratedDocument, OutboxEvent, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-08-02.
...
1. Create shared contract in `packages/contracts/src/story-08-03/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `GeneratedDocument, OutboxEvent, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `DocumentController` and `DocumentService` at the route `/api/v1/assessments/{assessmentId}/documents`.
...
6. Add outbox producer for `command.document.requested.v1` when the story needs async work; otherwise document `NO_ASYNC_COMMAND` in the service.
7. Add worker consumer/handler for `command.document.requested.v1` when applicable; consumer emits `event.document.generated.v1`.
...
10. Run manual smoke test and confirm observable output: ComplianceDocument metadata and GeneratedDocumentFile ref.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `VerifiedProfile, classification, gap analysis, citations`.
→ API route: `/api/v1/assessments/{assessmentId}/documents`.
...
→ service: `DocumentService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `GeneratedDocument, OutboxEvent, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
→ audit event: append `AuditEvent` with `eventId`, `correlationId`, `causationId`, actor, action and safe metadata.
→ outbox event: publish `command.document.requested.v1` if async processing is required; payload uses the envelope in `11-rabbitmq-topology-and-event-contracts.md`.
→ worker: Node.js worker handler consumes `command.document.requested.v1` and calls package `@langchain/langgraph`.
→ output state: API/UI exposes ComplianceDocument metadata and GeneratedDocumentFile ref; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `GeneratedDocument, OutboxEvent, AuditEvent` rows.
- RabbitMQ emits or consumes `command.document.requested.v1` / `event.document.generated.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: ComplianceDocument metadata and GeneratedDocumentFile ref.
