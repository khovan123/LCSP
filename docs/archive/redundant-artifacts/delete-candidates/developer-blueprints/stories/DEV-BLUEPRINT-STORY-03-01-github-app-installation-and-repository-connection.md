# DEV-BLUEPRINT — STORY-03-01 — GitHub App Installation and Repository Connection

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **GitHub App Installation and Repository Connection** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/github-integration` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/github-integration` | Manager-facing action, loading/error/blocked state |
| API | `github-integration` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.github_integration` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | GitHubRepositoryConnection, RepositorySnapshot, AuditEvent |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/github-integration/github-app-installation-and-repository-connection.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/github-integration/github-app-installation-and-repository-connection.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/github-integration/dto/github-app-installation-and-repository-connection.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/github-integration/repositories/github-app-installation-and-repository-connection.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/github_integration/github_app_installation_and_repository_connection-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/github_integration/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend GitHubRepositoryConnection, RepositorySnapshot, AuditEvent models |
| CREATE | `packages/contracts/src/github-integration/github-app-installation-and-repository-connection.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/github-integration/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/github-integration/github-app-installation-and-repository-connection.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/github-integration` or story-specific nested route.
- Form fields: IDs/references required for GitHub installation callback and repository connection request; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:03-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:03-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: GitHubRepositoryConnection, RepositorySnapshot, AuditEvent.
...
### UI Output
- Success state: show GitHubRepositoryConnection metadata and audit event.
...
Use Prisma migrations to add or extend GitHubRepositoryConnection, RepositorySnapshot, AuditEvent. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-02-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-02-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
- OAuth/OIDC identity is separate from GitHub App repository authorization.
...
|---|---|---|
| Implements GitHub App Installation and Repository Connection | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-02-03`.
...
This story turns `STORY-03-01` into an executable controlled-prototype slice. The business outcome is: GitHubRepositoryConnection metadata and audit event. It gives the developer exact ownership for package `@octokit/app, @octokit/rest`, API route `/api/v1/assessments/{assessmentId}/github/repository-connections`, service `GitHubRepositoryService`, and persistence models `RepositoryConnection, RepositorySnapshot`.

### Before coding

- Required completed predecessor stories: STORY-02-03.
...
1. Create shared contract in `packages/contracts/src/story-03-01/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `RepositoryConnection, RepositorySnapshot` if the model does not already exist.
3. Create or update NestJS module files for `GitHubController` and `GitHubRepositoryService` at the route `/api/v1/assessments/{assessmentId}/github/repository-connections`.
...
10. Run manual smoke test and confirm observable output: GitHubRepositoryConnection metadata and audit event.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `GitHub installation callback and repository connection request`.
→ API route: `/api/v1/assessments/{assessmentId}/github/repository-connections`.
...
→ service: `GitHubRepositoryService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `RepositoryConnection, RepositorySnapshot` using UUIDv7 internal IDs and reference-only payloads.
...
→ worker: Node.js worker handler consumes `none` and calls package `@octokit/app, @octokit/rest`.
→ output state: API/UI exposes GitHubRepositoryConnection metadata and audit event; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `RepositoryConnection, RepositorySnapshot` rows.
...
- Manager UI shows the expected visible result: GitHubRepositoryConnection metadata and audit event.
