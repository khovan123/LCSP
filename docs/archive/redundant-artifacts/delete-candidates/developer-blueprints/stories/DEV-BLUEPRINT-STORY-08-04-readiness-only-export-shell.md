# DEV-BLUEPRINT — STORY-08-04 — Readiness-only Export Shell

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Readiness-only Export Shell** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/classification/readiness-only-export-shell.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/classification/readiness-only-export-shell.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/classification/dto/readiness-only-export-shell.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/classification/repositories/readiness-only-export-shell.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/classification/readiness_only_export_shell-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/classification/readiness-only-export-shell.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/classification/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/classification/readiness-only-export-shell.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/classification` or story-specific nested route.
- Form fields: IDs/references required for Document/report export request; no raw source, secrets or full prompts.
...
- Route: `/api/v1/documents/{assessmentId}/export`.
...
```json
{
  ...
  "inputReference": "ref:08-04-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:08-04-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Prototype-only export with readiness warning.
...
3. Load predecessor state required by `STORY-08-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-08-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Readiness-only Export Shell | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-08-03`.
...
This story turns `STORY-08-04` into an executable controlled-prototype slice. The business outcome is: Prototype-only export with readiness warning. It gives the developer exact ownership for package `@langchain/langgraph`, API route `/api/v1/assessments/{assessmentId}/documents`, service `DocumentService`, and persistence models `GeneratedDocument, OutboxEvent, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-08-03.
...
1. Create shared contract in `packages/contracts/src/story-08-04/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Prototype-only export with readiness warning.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Document/report export request`.
...
→ output state: API/UI exposes Prototype-only export with readiness warning; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Prototype-only export with readiness warning.
