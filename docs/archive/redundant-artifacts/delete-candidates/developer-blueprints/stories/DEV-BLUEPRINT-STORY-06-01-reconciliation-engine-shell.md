# DEV-BLUEPRINT — STORY-06-01 — Reconciliation Engine Shell

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Reconciliation Engine Shell** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/reconciliation/reconciliation-engine-shell.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/reconciliation/reconciliation-engine-shell.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/reconciliation/dto/reconciliation-engine-shell.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/reconciliation/repositories/reconciliation-engine-shell.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/reconciliation/reconciliation_engine_shell-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/reconciliation/reconciliation-engine-shell.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/reconciliation/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/reconciliation/reconciliation-engine-shell.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/reconciliation` or story-specific nested route.
- Form fields: IDs/references required for WizardProfile, TechnicalProfile, AIUsageFlow; no raw source, secrets or full prompts.
...
- Route: `/api/v1/reconciliation/{assessmentId}/run`.
...
```json
{
  ...
  "inputReference": "ref:06-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:06-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show ConflictRecord or reconciled profile draft.
...
3. Load predecessor state required by `STORY-05-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-05-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Reconciliation Engine Shell | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-05-03`.
...
This story turns `STORY-06-01` into an executable controlled-prototype slice. The business outcome is: ConflictRecord or reconciled profile draft. It gives the developer exact ownership for package `@lcsp/reconciliation`, API route `/api/v1/assessments/{assessmentId}/reconciliation`, service `ReconciliationService`, and persistence models `ReconciliationConflict, VerifiedProfile, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-05-03.
...
1. Create shared contract in `packages/contracts/src/story-06-01/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `ReconciliationConflict, VerifiedProfile, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `ReconciliationController` and `ReconciliationService` at the route `/api/v1/assessments/{assessmentId}/reconciliation`.
...
7. Add worker consumer/handler for `command.reconciliation.requested.v1` when applicable; consumer emits `event.reconciliation.conflict-detected.v1`.
...
10. Run manual smoke test and confirm observable output: ConflictRecord or reconciled profile draft.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `WizardProfile, TechnicalProfile, AIUsageFlow`.
→ API route: `/api/v1/assessments/{assessmentId}/reconciliation`.
...
→ service: `ReconciliationService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `ReconciliationConflict, VerifiedProfile, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
...
→ output state: API/UI exposes ConflictRecord or reconciled profile draft; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `ReconciliationConflict, VerifiedProfile, AuditEvent` rows.
- RabbitMQ emits or consumes `command.reconciliation.requested.v1` / `event.reconciliation.conflict-detected.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: ConflictRecord or reconciled profile draft.
