# DEV-BLUEPRINT — STORY-09-04 — Prototype Observability and Manual Verification Support

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Prototype Observability and Manual Verification Support** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/security` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/security` | Manager-facing action, loading/error/blocked state |
| API | `security / observability` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.security` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | AuditEvent, ModelRunMetadata, TechnicalEvidenceReport |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/security/prototype-observability-and-manual-verification-support.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/security/prototype-observability-and-manual-verification-support.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/security/dto/prototype-observability-and-manual-verification-support.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/security/repositories/prototype-observability-and-manual-verification-support.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/security/prototype_observability_and_manual_verification_support-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/security/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend AuditEvent, ModelRunMetadata, TechnicalEvidenceReport models |
| CREATE | `packages/contracts/src/security/prototype-observability-and-manual-verification-support.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/security/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/security/prototype-observability-and-manual-verification-support.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/security` or story-specific nested route.
- Form fields: IDs/references required for Correlation ID and workflow state changes; no raw source, secrets or full prompts.
...
- Route: `/api/v1/security/verification`.
...
```json
{
  ...
  "inputReference": "ref:09-04-prototype",
...
- Routing key: `event.audit.created.v1` when async work is required.
- Job payload JSON:
```json
{
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:09-04-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: AuditEvent, ModelRunMetadata, TechnicalEvidenceReport.
...
### UI Output
- Success state: show Structured logs, metrics plan, audit records.
...
Use Prisma migrations to add or extend AuditEvent, ModelRunMetadata, TechnicalEvidenceReport. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-06-04` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-06-04` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
- Preserve controlled prototype guardrails and fail closed on missing authorization, evidence or prerequisites.
...
|---|---|---|
| Implements Prototype Observability and Manual Verification Support | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-06-04`.
- Intended wave: Wave 6.
...
This story turns `STORY-09-04` into an executable controlled-prototype slice. The business outcome is: Structured logs, metrics plan, audit records. It gives the developer exact ownership for package `@lcsp/logger, @lcsp/scanner`, API route `/api/v1/security/verification`, service `SecurityVerificationService`, and persistence models `AuditEvent, TechnicalEvidenceReport`.

### Before coding

- Required completed predecessor stories: STORY-06-04.
...
1. Create shared contract in `packages/contracts/src/story-09-04/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `AuditEvent, TechnicalEvidenceReport` if the model does not already exist.
3. Create or update NestJS module files for `SecurityVerificationController` and `SecurityVerificationService` at the route `/api/v1/security/verification`.
...
6. Add outbox producer for `none` when the story needs async work; otherwise document `NO_ASYNC_COMMAND` in the service.
...
10. Run manual smoke test and confirm observable output: Structured logs, metrics plan, audit records.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Correlation ID and workflow state changes`.
→ API route: `/api/v1/security/verification`.
...
→ service: `SecurityVerificationService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `AuditEvent, TechnicalEvidenceReport` using UUIDv7 internal IDs and reference-only payloads.
→ audit event: append `AuditEvent` with `eventId`, `correlationId`, `causationId`, actor, action and safe metadata.
→ outbox event: publish `none` if async processing is required; payload uses the envelope in `11-rabbitmq-topology-and-event-contracts.md`.
→ worker: Node.js worker handler consumes `none` and calls package `@lcsp/logger, @lcsp/scanner`.
→ output state: API/UI exposes Structured logs, metrics plan, audit records; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `AuditEvent, TechnicalEvidenceReport` rows.
- RabbitMQ emits or consumes `none` / `event.audit.created.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: Structured logs, metrics plan, audit records.
