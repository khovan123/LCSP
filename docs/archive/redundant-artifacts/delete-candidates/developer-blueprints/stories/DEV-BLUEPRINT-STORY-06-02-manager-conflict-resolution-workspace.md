# DEV-BLUEPRINT — STORY-06-02 — Manager Conflict Resolution Workspace

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Manager Conflict Resolution Workspace** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/reconciliation/manager-conflict-resolution-workspace.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/reconciliation/manager-conflict-resolution-workspace.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/reconciliation/dto/manager-conflict-resolution-workspace.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/reconciliation/repositories/manager-conflict-resolution-workspace.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/reconciliation/manager_conflict_resolution_workspace-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/reconciliation/manager-conflict-resolution-workspace.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/reconciliation/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/reconciliation/manager-conflict-resolution-workspace.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/reconciliation` or story-specific nested route.
- Form fields: IDs/references required for Manager conflict resolution action; no raw source, secrets or full prompts.
...
- Route: `/api/v1/conflicts/{conflict_id}/resolve`.
...
```json
{
  ...
  "inputReference": "ref:06-02-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:06-02-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show ConflictResolution and audit event.
...
3. Load predecessor state required by `STORY-06-01` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-06-01` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Manager Conflict Resolution Workspace | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-06-01`.
...
This story turns `STORY-06-02` into an executable controlled-prototype slice. The business outcome is: ConflictResolution and audit event. It gives the developer exact ownership for package `@lcsp/reconciliation`, API route `/api/v1/assessments/{assessmentId}/reconciliation/conflicts/{conflictId}/resolution`, service `ConflictResolutionService`, and persistence models `ConflictResolution, VerifiedProfile, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-06-01.
...
1. Create shared contract in `packages/contracts/src/story-06-02/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `ConflictResolution, VerifiedProfile, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `ConflictController` and `ConflictResolutionService` at the route `/api/v1/assessments/{assessmentId}/reconciliation/conflicts/{conflictId}/resolution`.
...
10. Run manual smoke test and confirm observable output: ConflictResolution and audit event.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Manager conflict resolution action`.
→ API route: `/api/v1/assessments/{assessmentId}/reconciliation/conflicts/{conflictId}/resolution`.
...
→ service: `ConflictResolutionService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `ConflictResolution, VerifiedProfile, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
...
→ output state: API/UI exposes ConflictResolution and audit event; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `ConflictResolution, VerifiedProfile, AuditEvent` rows.
...
- Manager UI shows the expected visible result: ConflictResolution and audit event.
