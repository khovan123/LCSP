# DEV-BLUEPRINT — STORY-08-01 — Risk Classification Workflow Shell

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Risk Classification Workflow Shell** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/classification/risk-classification-workflow-shell.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/classification/risk-classification-workflow-shell.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/classification/dto/risk-classification-workflow-shell.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/classification/repositories/risk-classification-workflow-shell.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/classification/risk_classification_workflow_shell-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/classification/risk-classification-workflow-shell.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/classification/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/classification/risk-classification-workflow-shell.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/classification` or story-specific nested route.
- Form fields: IDs/references required for VerifiedProfile and legal citation refs; no raw source, secrets or full prompts.
...
- Route: `/api/v1/classification/{assessmentId}/run`.
...
```json
{
  ...
  "inputReference": "ref:08-01-prototype",
...
- Routing key: `command.classification.requested.v1` when async work is required.
- Job payload JSON:
```json
{
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:08-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show RiskClassificationResult or blocked state.
...
3. Load predecessor state required by `STORY-07-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-07-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Risk Classification Workflow Shell | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-07-03`.
...
This story turns `STORY-08-01` into an executable controlled-prototype slice. The business outcome is: RiskClassificationResult or blocked state. It gives the developer exact ownership for package `@langchain/langgraph`, API route `/api/v1/assessments/{assessmentId}/classification`, service `ClassificationService`, and persistence models `RiskClassification, OutboxEvent, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-07-03.
...
1. Create shared contract in `packages/contracts/src/story-08-01/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `RiskClassification, OutboxEvent, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `ClassificationController` and `ClassificationService` at the route `/api/v1/assessments/{assessmentId}/classification`.
...
6. Add outbox producer for `command.classification.requested.v1` when the story needs async work; otherwise document `NO_ASYNC_COMMAND` in the service.
7. Add worker consumer/handler for `command.classification.requested.v1` when applicable; consumer emits `event.classification.completed.v1`.
...
10. Run manual smoke test and confirm observable output: RiskClassificationResult or blocked state.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `VerifiedProfile and legal citation refs`.
→ API route: `/api/v1/assessments/{assessmentId}/classification`.
...
→ service: `ClassificationService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `RiskClassification, OutboxEvent, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
→ audit event: append `AuditEvent` with `eventId`, `correlationId`, `causationId`, actor, action and safe metadata.
→ outbox event: publish `command.classification.requested.v1` if async processing is required; payload uses the envelope in `11-rabbitmq-topology-and-event-contracts.md`.
→ worker: Node.js worker handler consumes `command.classification.requested.v1` and calls package `@langchain/langgraph`.
→ output state: API/UI exposes RiskClassificationResult or blocked state; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `RiskClassification, OutboxEvent, AuditEvent` rows.
- RabbitMQ emits or consumes `command.classification.requested.v1` / `event.classification.completed.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: RiskClassificationResult or blocked state.
