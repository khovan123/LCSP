# DEV-BLUEPRINT — STORY-04-04 — Evidence Gates and TechnicalProfile Builder

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Evidence Gates and TechnicalProfile Builder** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/repository-scan/evidence-gates-and-technical-profile-builder.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/repository-scan/evidence-gates-and-technical-profile-builder.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/repository-scan/dto/evidence-gates-and-technical-profile-builder.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/repository-scan/repositories/evidence-gates-and-technical-profile-builder.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/repository_scan/evidence_gates_and_technical_profile_builder-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/repository-scan/evidence-gates-and-technical-profile-builder.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/repository-scan/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/repository-scan/evidence-gates-and-technical-profile-builder.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/repository-scan` or story-specific nested route.
- Form fields: IDs/references required for TechnicalEvidenceReport; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:04-04-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:04-04-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Schema Gate, Quality Gate, TechnicalProfile.
...
3. Load predecessor state required by `STORY-04-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-04-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Evidence Gates and TechnicalProfile Builder | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-04-03`.
...
This story turns `STORY-04-04` into an executable controlled-prototype slice. The business outcome is: Schema Gate, Quality Gate, TechnicalProfile. It gives the developer exact ownership for package `@lcsp/scanner`, API route `/api/v1/assessments/{assessmentId}/technical-evidence-report`, service `EvidenceService`, and persistence models `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile`.

### Before coding

- Required completed predecessor stories: STORY-04-03.
...
1. Create shared contract in `packages/contracts/src/story-04-04/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Schema Gate, Quality Gate, TechnicalProfile.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `TechnicalEvidenceReport`.
...
→ output state: API/UI exposes Schema Gate, Quality Gate, TechnicalProfile; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Schema Gate, Quality Gate, TechnicalProfile.
