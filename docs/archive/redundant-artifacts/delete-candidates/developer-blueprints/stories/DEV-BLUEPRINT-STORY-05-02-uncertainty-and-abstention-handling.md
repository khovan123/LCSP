# DEV-BLUEPRINT — STORY-05-02 — Uncertainty and Abstention Handling

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Uncertainty and Abstention Handling** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/ai-usage-flow/uncertainty-and-abstention-handling.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/ai-usage-flow/uncertainty-and-abstention-handling.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/ai-usage-flow/dto/uncertainty-and-abstention-handling.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/ai-usage-flow/repositories/uncertainty-and-abstention-handling.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/ai_usage_flow/uncertainty_and_abstention_handling-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/ai-usage-flow/uncertainty-and-abstention-handling.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/ai-usage-flow/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/ai-usage-flow/uncertainty-and-abstention-handling.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/ai-usage-flow` or story-specific nested route.
- Form fields: IDs/references required for Unsupported or unclear analysis boundary; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:05-02-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:05-02-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Uncertainty reasons, coverage limitation, blocked eligibility.
...
3. Load predecessor state required by `STORY-05-01` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-05-01` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Uncertainty and Abstention Handling | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-05-01`.
...
This story turns `STORY-05-02` into an executable controlled-prototype slice. The business outcome is: Uncertainty reasons and blocked eligibility. It gives the developer exact ownership for package `@lcsp/ai-usage-flow`, API route `/api/v1/assessments/{assessmentId}/ai-usage-flow`, service `AIUsageFlowService`, and persistence models `AIUsageFlow, AIUsageFlowClaim, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-05-01.
...
1. Create shared contract in `packages/contracts/src/story-05-02/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Uncertainty reasons and blocked eligibility.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Unsupported or unclear analysis boundary`.
...
→ output state: API/UI exposes Uncertainty reasons and blocked eligibility; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Uncertainty reasons and blocked eligibility.
