# DEV-BLUEPRINT — STORY-09-01 — Secret Redaction and Evidence Sanitization

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Secret Redaction and Evidence Sanitization** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/security/secret-redaction-and-evidence-sanitization.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/security/secret-redaction-and-evidence-sanitization.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/security/dto/secret-redaction-and-evidence-sanitization.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/security/repositories/secret-redaction-and-evidence-sanitization.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/security/secret_redaction_and_evidence_sanitization-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/security/secret-redaction-and-evidence-sanitization.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/security/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/security/secret-redaction-and-evidence-sanitization.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/security` or story-specific nested route.
- Form fields: IDs/references required for Potential secret-bearing text or finding; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:09-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:09-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Redacted evidence payload and audit metadata.
...
|---|---|---|
| Implements Secret Redaction and Evidence Sanitization | API/Web/Worker files in plan | Unit + integration tests |
...
This story turns `STORY-09-01` into an executable controlled-prototype slice. The business outcome is: Redacted evidence payload and audit metadata. It gives the developer exact ownership for package `@lcsp/logger, @lcsp/scanner`, API route `/api/v1/security/verification`, service `SecurityVerificationService`, and persistence models `AuditEvent, TechnicalEvidenceReport`.
...
1. Create shared contract in `packages/contracts/src/story-09-01/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Redacted evidence payload and audit metadata.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Potential secret-bearing text or finding`.
...
→ output state: API/UI exposes Redacted evidence payload and audit metadata; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Redacted evidence payload and audit metadata.
