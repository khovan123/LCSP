# DEV-BLUEPRINT — STORY-07-03 — Citation Guardrail Shell

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Citation Guardrail Shell** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/legal-rag/citation-guardrail-shell.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/legal-rag/citation-guardrail-shell.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/legal-rag/dto/citation-guardrail-shell.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/legal-rag/repositories/citation-guardrail-shell.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/legal_rag/citation_guardrail_shell-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/legal-rag/citation-guardrail-shell.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/legal-rag/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/legal-rag/citation-guardrail-shell.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/legal-rag` or story-specific nested route.
- Form fields: IDs/references required for Classification/legal conclusion candidate; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:07-03-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:07-03-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Guardrail pass or citation-missing block.
...
3. Load predecessor state required by `STORY-07-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-07-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Citation Guardrail Shell | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-07-02`.
...
This story turns `STORY-07-03` into an executable controlled-prototype slice. The business outcome is: Guardrail pass or citation-missing block. It gives the developer exact ownership for package `@lcsp/legal-rag, chromadb`, API route `/api/v1/assessments/{assessmentId}/legal-rule-matches`, service `LegalRagService`, and persistence models `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-07-02.
...
1. Create shared contract in `packages/contracts/src/story-07-03/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Guardrail pass or citation-missing block.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Classification/legal conclusion candidate`.
...
→ output state: API/UI exposes Guardrail pass or citation-missing block; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Guardrail pass or citation-missing block.
