# DEV-BLUEPRINT — STORY-04-01 — Scan Job Lifecycle and Idempotency

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Scan Job Lifecycle and Idempotency** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/repository-scan/scan-job-lifecycle-and-idempotency.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/repository-scan/scan-job-lifecycle-and-idempotency.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/repository-scan/dto/scan-job-lifecycle-and-idempotency.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/repository-scan/repositories/scan-job-lifecycle-and-idempotency.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/repository_scan/scan_job_lifecycle_and_idempotency-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/repository-scan/scan-job-lifecycle-and-idempotency.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/repository-scan/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/repository-scan/scan-job-lifecycle-and-idempotency.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/repository-scan` or story-specific nested route.
- Form fields: IDs/references required for Manager scan request for repository snapshot; no raw source, secrets or full prompts.
...
- Route: `/api/v1/assessments/{assessmentId}/repository-scans`.
...
```json
{
  ...
  "inputReference": "ref:04-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:04-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show RepositoryScanJob, RabbitMQ job, progress states.
...
3. Load predecessor state required by `STORY-03-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-03-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Scan Job Lifecycle and Idempotency | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-03-02`.
- Intended wave: Wave 3.
...
This story turns `STORY-04-01` into an executable controlled-prototype slice. The business outcome is: RepositoryScanJob, RabbitMQ job, progress states. It gives the developer exact ownership for package `amqplib`, API route `/api/v1/assessments/{assessmentId}/scans`, service `ScanService`, and persistence models `RepositoryScanJob, OutboxEvent, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-03-02.
...
1. Create shared contract in `packages/contracts/src/story-04-01/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `RepositoryScanJob, OutboxEvent, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `ScanController` and `ScanService` at the route `/api/v1/assessments/{assessmentId}/scans`.
...
7. Add worker consumer/handler for `command.scan.requested.v1` when applicable; consumer emits `event.scan.failed.v1`.
...
10. Run manual smoke test and confirm observable output: RepositoryScanJob, RabbitMQ job, progress states.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Manager scan request for repository snapshot`.
→ API route: `/api/v1/assessments/{assessmentId}/scans`.
...
→ service: `ScanService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `RepositoryScanJob, OutboxEvent, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
...
→ worker: Node.js worker handler consumes `command.scan.requested.v1` and calls package `amqplib`.
→ output state: API/UI exposes RepositoryScanJob, RabbitMQ job, progress states; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `RepositoryScanJob, OutboxEvent, AuditEvent` rows.
- RabbitMQ emits or consumes `command.scan.requested.v1` / `event.scan.failed.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: RepositoryScanJob, RabbitMQ job, progress states.
