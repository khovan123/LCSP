# DEV-BLUEPRINT — STORY-07-02 — Legal Rule and Citation Retrieval Boundary

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Legal Rule and Citation Retrieval Boundary** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/legal-rag` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/legal-rag` | Manager-facing action, loading/error/blocked state |
| API | `legal-rag` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.legal_rag` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/legal-rag/legal-rule-and-citation-retrieval-boundary.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/legal-rag/legal-rule-and-citation-retrieval-boundary.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/legal-rag/dto/legal-rule-and-citation-retrieval-boundary.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/legal-rag/repositories/legal-rule-and-citation-retrieval-boundary.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/legal_rag/legal_rule_and_citation_retrieval_boundary-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/legal_rag/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent models |
| CREATE | `packages/contracts/src/legal-rag/legal-rule-and-citation-retrieval-boundary.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/legal-rag/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/legal-rag/legal-rule-and-citation-retrieval-boundary.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/legal-rag` or story-specific nested route.
- Form fields: IDs/references required for VerifiedProfile and AIUsageFlow claim refs; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:07-02-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:07-02-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent.
...
### UI Output
- Success state: show Citation candidates and retrieval trace.
...
Use Prisma migrations to add or extend LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-07-01` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-07-01` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Legal Rule and Citation Retrieval Boundary | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-07-01`.
...
This story turns `STORY-07-02` into an executable controlled-prototype slice. The business outcome is: Citation candidates and retrieval trace. It gives the developer exact ownership for package `@lcsp/legal-rag, chromadb`, API route `/api/v1/assessments/{assessmentId}/legal-rule-matches`, service `LegalRagService`, and persistence models `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-07-01.
...
1. Create shared contract in `packages/contracts/src/story-07-02/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `LegalRagController` and `LegalRagService` at the route `/api/v1/assessments/{assessmentId}/legal-rule-matches`.
...
7. Add worker consumer/handler for `command.classification.requested.v1` when applicable; consumer emits `event.classification.blocked.v1`.
...
10. Run manual smoke test and confirm observable output: Citation candidates and retrieval trace.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `VerifiedProfile and AIUsageFlow claim refs`.
→ API route: `/api/v1/assessments/{assessmentId}/legal-rule-matches`.
...
→ service: `LegalRagService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
...
→ worker: Node.js worker handler consumes `command.classification.requested.v1` and calls package `@lcsp/legal-rag, chromadb`.
→ output state: API/UI exposes Citation candidates and retrieval trace; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent` rows.
- RabbitMQ emits or consumes `command.classification.requested.v1` / `event.classification.blocked.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: Citation candidates and retrieval trace.
