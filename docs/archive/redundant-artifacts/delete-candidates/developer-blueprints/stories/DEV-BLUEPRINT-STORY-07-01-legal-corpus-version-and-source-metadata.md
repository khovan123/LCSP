# DEV-BLUEPRINT — STORY-07-01 — Legal Corpus Version and Source Metadata

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Legal Corpus Version and Source Metadata** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/legal-rag/legal-corpus-version-and-source-metadata.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/legal-rag/legal-corpus-version-and-source-metadata.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/legal-rag/dto/legal-corpus-version-and-source-metadata.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/legal-rag/repositories/legal-corpus-version-and-source-metadata.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/legal_rag/legal_corpus_version_and_source_metadata-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/legal-rag/legal-corpus-version-and-source-metadata.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/legal-rag/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/legal-rag/legal-corpus-version-and-source-metadata.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/legal-rag` or story-specific nested route.
- Form fields: IDs/references required for Legal corpus source metadata; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:07-01-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:07-01-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show LegalDocumentVersion and LegalRule metadata.
...
3. Load predecessor state required by `STORY-06-03` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-06-03` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Legal Corpus Version and Source Metadata | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-06-03`.
...
This story turns `STORY-07-01` into an executable controlled-prototype slice. The business outcome is: LegalDocumentVersion and LegalRule metadata. It gives the developer exact ownership for package `@lcsp/legal-rag, chromadb`, API route `/api/v1/legal-corpus`, service `LegalRagService`, and persistence models `LegalDocumentVersion, LegalRule, LegalCitation, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-06-03.
...
1. Create shared contract in `packages/contracts/src/story-07-01/` with DTO, response, error and event payload schemas.
...
3. Create or update NestJS module files for `LegalRagController` and `LegalRagService` at the route `/api/v1/legal-corpus`.
...
10. Run manual smoke test and confirm observable output: LegalDocumentVersion and LegalRule metadata.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Legal corpus source metadata`.
→ API route: `/api/v1/legal-corpus`.
...
→ output state: API/UI exposes LegalDocumentVersion and LegalRule metadata; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: LegalDocumentVersion and LegalRule metadata.
