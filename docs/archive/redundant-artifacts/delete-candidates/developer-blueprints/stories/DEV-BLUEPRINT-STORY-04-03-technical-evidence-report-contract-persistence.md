# DEV-BLUEPRINT — STORY-04-03 — TechnicalEvidenceReport Contract Persistence

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **TechnicalEvidenceReport Contract Persistence** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/repository-scan` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/repository-scan` | Manager-facing action, loading/error/blocked state |
| API | `repository-scan / evidence` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.repository_scan` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | RepositoryScanJob, TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, EvidenceGateResult, TechnicalProfile, AuditEvent |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/repository-scan/technical-evidence-report-contract-persistence.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/repository-scan/technical-evidence-report-contract-persistence.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/repository-scan/dto/technical-evidence-report-contract-persistence.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/repository-scan/repositories/technical-evidence-report-contract-persistence.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/repository_scan/technical_evidence_report_contract_persistence-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/repository_scan/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend RepositoryScanJob, TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, EvidenceGateResult, TechnicalProfile, AuditEvent models |
| CREATE | `packages/contracts/src/repository-scan/technical-evidence-report-contract-persistence.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/repository-scan/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/repository-scan/technical-evidence-report-contract-persistence.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/repository-scan` or story-specific nested route.
- Form fields: IDs/references required for Normalized scanner output; no raw source, secrets or full prompts.
...
- Route: `/api/v1/evidence/{assessmentId}`.
...
```json
{
  ...
  "inputReference": "ref:04-03-prototype",
...
- Routing key: `command.scan.requested.v1` when async work is required.
- Job payload JSON:
```json
{
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:04-03-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: RepositoryScanJob, TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, EvidenceGateResult, TechnicalProfile, AuditEvent.
...
### UI Output
- Success state: show TechnicalEvidenceReport with hash and evidence refs.
...
Use Prisma migrations to add or extend RepositoryScanJob, TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, EvidenceGateResult, TechnicalProfile, AuditEvent. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-04-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-04-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
- Scanner work is static-analysis only and must not execute customer code, scripts, installs, builds, tests, Docker, CI workflows or API probes.
...
|---|---|---|
| Implements TechnicalEvidenceReport Contract Persistence | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-04-02`.
...
This story turns `STORY-04-03` into an executable controlled-prototype slice. The business outcome is: TechnicalEvidenceReport with hash and evidence refs. It gives the developer exact ownership for package `@lcsp/scanner`, API route `/api/v1/assessments/{assessmentId}/technical-evidence-report`, service `EvidenceService`, and persistence models `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile`.

### Before coding

- Required completed predecessor stories: STORY-04-02.
...
1. Create shared contract in `packages/contracts/src/story-04-03/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile` if the model does not already exist.
3. Create or update NestJS module files for `EvidenceController` and `EvidenceService` at the route `/api/v1/assessments/{assessmentId}/technical-evidence-report`.
...
6. Add outbox producer for `command.scan.requested.v1` when the story needs async work; otherwise document `NO_ASYNC_COMMAND` in the service.
7. Add worker consumer/handler for `command.scan.requested.v1` when applicable; consumer emits `event.scan.completed.v1`.
...
10. Run manual smoke test and confirm observable output: TechnicalEvidenceReport with hash and evidence refs.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Normalized scanner output`.
→ API route: `/api/v1/assessments/{assessmentId}/technical-evidence-report`.
...
→ service: `EvidenceService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile` using UUIDv7 internal IDs and reference-only payloads.
→ audit event: append `AuditEvent` with `eventId`, `correlationId`, `causationId`, actor, action and safe metadata.
→ outbox event: publish `command.scan.requested.v1` if async processing is required; payload uses the envelope in `11-rabbitmq-topology-and-event-contracts.md`.
→ worker: Node.js worker handler consumes `command.scan.requested.v1` and calls package `@lcsp/scanner`.
→ output state: API/UI exposes TechnicalEvidenceReport with hash and evidence refs; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile` rows.
- RabbitMQ emits or consumes `command.scan.requested.v1` / `event.scan.completed.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: TechnicalEvidenceReport with hash and evidence refs.
