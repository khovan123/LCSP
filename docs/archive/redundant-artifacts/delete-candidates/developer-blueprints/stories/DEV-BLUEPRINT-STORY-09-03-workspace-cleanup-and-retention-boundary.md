# DEV-BLUEPRINT — STORY-09-03 — Workspace Cleanup and Retention Boundary

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Workspace Cleanup and Retention Boundary** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/security/workspace-cleanup-and-retention-boundary.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/security/workspace-cleanup-and-retention-boundary.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/security/dto/workspace-cleanup-and-retention-boundary.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/security/repositories/workspace-cleanup-and-retention-boundary.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/security/workspace_cleanup_and_retention_boundary-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/security/workspace-cleanup-and-retention-boundary.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/security/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/security/workspace-cleanup-and-retention-boundary.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/security` or story-specific nested route.
- Form fields: IDs/references required for Temporary scanner workspace path; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:09-03-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:09-03-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Cleanup confirmation and retention audit.
...
|---|---|---|
| Implements Workspace Cleanup and Retention Boundary | API/Web/Worker files in plan | Unit + integration tests |
...
This story turns `STORY-09-03` into an executable controlled-prototype slice. The business outcome is: Cleanup confirmation and retention audit. It gives the developer exact ownership for package `@lcsp/logger, @lcsp/scanner`, API route `/api/v1/security/verification`, service `SecurityVerificationService`, and persistence models `AuditEvent, TechnicalEvidenceReport`.
...
1. Create shared contract in `packages/contracts/src/story-09-03/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Cleanup confirmation and retention audit.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Temporary scanner workspace path`.
...
→ output state: API/UI exposes Cleanup confirmation and retention audit; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Cleanup confirmation and retention audit.
