# DEV-BLUEPRINT — STORY-03-02 — Repository, Branch, and Commit Selection

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Repository, Branch, and Commit Selection** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/github-integration/repository-branch-and-commit-selection.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/github-integration/repository-branch-and-commit-selection.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/github-integration/dto/repository-branch-and-commit-selection.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/github-integration/repositories/repository-branch-and-commit-selection.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/github_integration/repository_branch_and_commit_selection-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/github-integration/repository-branch-and-commit-selection.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/github-integration/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/github-integration/repository-branch-and-commit-selection.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/github-integration` or story-specific nested route.
- Form fields: IDs/references required for Manager repository branch commit selection; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:03-02-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:03-02-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show RepositorySnapshot metadata and scan-ready state.
...
3. Load predecessor state required by `STORY-03-01` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-03-01` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Repository, Branch, and Commit Selection | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-03-01`.
...
This story turns `STORY-03-02` into an executable controlled-prototype slice. The business outcome is: RepositorySnapshot metadata and scan-ready state. It gives the developer exact ownership for package `@octokit/app, @octokit/rest`, API route `/api/v1/assessments/{assessmentId}/github/repository-snapshots`, service `GitHubRepositoryService`, and persistence models `RepositoryConnection, RepositorySnapshot`.

### Before coding

- Required completed predecessor stories: STORY-03-01.
...
1. Create shared contract in `packages/contracts/src/story-03-02/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `RepositoryConnection, RepositorySnapshot` if the model does not already exist.
3. Create or update NestJS module files for `GitHubController` and `GitHubRepositoryService` at the route `/api/v1/assessments/{assessmentId}/github/repository-snapshots`.
...
10. Run manual smoke test and confirm observable output: RepositorySnapshot metadata and scan-ready state.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Manager repository branch commit selection`.
→ API route: `/api/v1/assessments/{assessmentId}/github/repository-snapshots`.
...
→ output state: API/UI exposes RepositorySnapshot metadata and scan-ready state; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: RepositorySnapshot metadata and scan-ready state.
