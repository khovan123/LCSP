# DEV-BLUEPRINT — STORY-05-03 — Human Review and Automation Level Mapping

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Human Review and Automation Level Mapping** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
| API | NestJS + TypeScript | `apps/api/src/modules/ai-usage-flow` | REST JSON, guards, transactions and audit emission |
...
|---|---|---|
| Web | `apps/web/src/features/ai-usage-flow` | Manager-facing action, loading/error/blocked state |
| API | `ai-usage-flow` | Request validation, authorization, transaction, audit, event publication |
| Worker | `worker.handlers.ai_usage_flow` | Async node processing where required |
| Persistence | Prisma/PostgreSQL | AIUsageFlow, AIUsageFlowClaim, EvidenceReference, AuditEvent |

## 4. Exact File Plan
| Action | Exact Path | Purpose |
|---|---|---|
| CREATE | `apps/api/src/modules/ai-usage-flow/human-review-and-automation-level-mapping.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/ai-usage-flow/human-review-and-automation-level-mapping.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/ai-usage-flow/dto/human-review-and-automation-level-mapping.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/ai-usage-flow/repositories/human-review-and-automation-level-mapping.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/ai_usage_flow/human_review_and_automation_level_mapping-handler.ts` | Node worker handler for queue/orchestrator processing |
| CREATE | `apps/worker/src/ai_usage_flow/schemas.ts` | Zod input/output schemas |
| MODIFY | `prisma/schema.prisma` | Add or extend AIUsageFlow, AIUsageFlowClaim, EvidenceReference, AuditEvent models |
| CREATE | `packages/contracts/src/ai-usage-flow/human-review-and-automation-level-mapping.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/ai-usage-flow/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/ai-usage-flow/human-review-and-automation-level-mapping.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/ai-usage-flow` or story-specific nested route.
- Form fields: IDs/references required for AIUsageFlow signals and evidence refs; no raw source, secrets or full prompts.
...
- Route: `/api/v1/ai-usage-flows/{assessmentId}`.
...
```json
{
  ...
  "inputReference": "ref:05-03-prototype",
...
- Routing key: `command.ai-usage-flow.requested.v1` when async work is required.
- Job payload JSON:
```json
{
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:05-03-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### Database Output
- Models changed: AIUsageFlow, AIUsageFlowClaim, EvidenceReference, AuditEvent.
...
### UI Output
- Success state: show human_review and automation_level claim values.
...
Use Prisma migrations to add or extend AIUsageFlow, AIUsageFlowClaim, EvidenceReference, AuditEvent. Store IDs, hashes, refs, status, timestamps, version fields and audit correlation IDs. Do not persist raw source, full prompts, secrets or full AST bodies.
...
3. Load predecessor state required by `STORY-05-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-05-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
- LLM Gateway receives only sanitized structured metadata; raw source, full prompts, secrets and raw AST bodies are forbidden.
...
|---|---|---|
| Implements Human Review and Automation Level Mapping | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-05-02`.
...
This story turns `STORY-05-03` into an executable controlled-prototype slice. The business outcome is: human_review and automation_level claims. It gives the developer exact ownership for package `@lcsp/ai-usage-flow`, API route `/api/v1/assessments/{assessmentId}/ai-usage-flow`, service `AIUsageFlowService`, and persistence models `AIUsageFlow, AIUsageFlowClaim, AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-05-02.
...
1. Create shared contract in `packages/contracts/src/story-05-03/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `AIUsageFlow, AIUsageFlowClaim, AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `AIUsageFlowController` and `AIUsageFlowService` at the route `/api/v1/assessments/{assessmentId}/ai-usage-flow`.
...
6. Add outbox producer for `command.ai-usage-flow.requested.v1` when the story needs async work; otherwise document `NO_ASYNC_COMMAND` in the service.
7. Add worker consumer/handler for `command.ai-usage-flow.requested.v1` when applicable; consumer emits `event.ai-usage-flow.completed.v1`.
...
10. Run manual smoke test and confirm observable output: human_review and automation_level claims.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `AIUsageFlow signals and evidence refs`.
→ API route: `/api/v1/assessments/{assessmentId}/ai-usage-flow`.
...
→ service: `AIUsageFlowService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `AIUsageFlow, AIUsageFlowClaim, AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
→ audit event: append `AuditEvent` with `eventId`, `correlationId`, `causationId`, actor, action and safe metadata.
→ outbox event: publish `command.ai-usage-flow.requested.v1` if async processing is required; payload uses the envelope in `11-rabbitmq-topology-and-event-contracts.md`.
→ worker: Node.js worker handler consumes `command.ai-usage-flow.requested.v1` and calls package `@lcsp/ai-usage-flow`.
→ output state: API/UI exposes human_review and automation_level claims; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `AIUsageFlow, AIUsageFlowClaim, AuditEvent` rows.
- RabbitMQ emits or consumes `command.ai-usage-flow.requested.v1` / `event.ai-usage-flow.completed.v1` only when applicable.
- Audit table contains immutable event for the state-changing operation.
- Manager UI shows the expected visible result: human_review and automation_level claims.
