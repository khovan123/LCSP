# DEV-BLUEPRINT — STORY-04-02 — Synthetic Fixture-driven Scanner Prototype

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Synthetic Fixture-driven Scanner Prototype** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/repository-scan/synthetic-fixture-driven-scanner-prototype.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/repository-scan/synthetic-fixture-driven-scanner-prototype.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/repository-scan/dto/synthetic-fixture-driven-scanner-prototype.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/repository-scan/repositories/synthetic-fixture-driven-scanner-prototype.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/repository_scan/synthetic_fixture_driven_scanner_prototype-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/repository-scan/synthetic-fixture-driven-scanner-prototype.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/repository-scan/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/repository-scan/synthetic-fixture-driven-scanner-prototype.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/repository-scan` or story-specific nested route.
- Form fields: IDs/references required for Approved synthetic fixture reference; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:04-02-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:04-02-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Normalized TechnicalFinding and scanner limitations.
...
3. Load predecessor state required by `STORY-04-01` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-04-01` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Synthetic Fixture-driven Scanner Prototype | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-04-01`.
...
This story turns `STORY-04-02` into an executable controlled-prototype slice. The business outcome is: Normalized TechnicalFinding and scanner limitations. It gives the developer exact ownership for package `@lcsp/scanner`, API route `/api/v1/assessments/{assessmentId}/technical-evidence-report`, service `EvidenceService`, and persistence models `TechnicalEvidenceReport, TechnicalFinding, EvidenceReference, TechnicalProfile`.

### Before coding

- Required completed predecessor stories: STORY-04-01.
...
1. Create shared contract in `packages/contracts/src/story-04-02/` with DTO, response, error and event payload schemas.
...
10. Run manual smoke test and confirm observable output: Normalized TechnicalFinding and scanner limitations.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Approved synthetic fixture reference`.
...
→ output state: API/UI exposes Normalized TechnicalFinding and scanner limitations; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- Manager UI shows the expected visible result: Normalized TechnicalFinding and scanner limitations.
