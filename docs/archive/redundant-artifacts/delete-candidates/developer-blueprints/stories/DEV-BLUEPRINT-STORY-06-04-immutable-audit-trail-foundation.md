# DEV-BLUEPRINT — STORY-06-04 — Immutable Audit Trail Foundation

## 1. Implementation Objective
Implement the controlled MVP prototype slice for **Immutable Audit Trail Foundation** so developers have exact service, data, API, event and verification guidance without claiming production readiness.
...
|---|---|---|
| CREATE | `apps/api/src/modules/reconciliation/immutable-audit-trail-foundation.controller.ts` | REST endpoint boundary |
| CREATE | `apps/api/src/modules/reconciliation/immutable-audit-trail-foundation.service.ts` | State validation, transaction and audit orchestration |
| CREATE | `apps/api/src/modules/reconciliation/dto/immutable-audit-trail-foundation.dto.ts` | Request/response DTOs and OpenAPI decorators |
| CREATE | `apps/api/src/modules/reconciliation/repositories/immutable-audit-trail-foundation.repository.ts` | Prisma data access boundary |
| CREATE | `apps/worker/src/reconciliation/immutable_audit_trail_foundation-handler.ts` | Node worker handler for queue/orchestrator processing |
...
| CREATE | `packages/contracts/src/reconciliation/immutable-audit-trail-foundation.ts` | Shared DTO/event/job contracts |
| CREATE | `apps/web/src/app/(app)/reconciliation/page.tsx` | Manager-facing route when UI is needed |
| CREATE | `apps/web/src/features/reconciliation/immutable-audit-trail-foundation.tsx` | Feature component/hook boundary |

## 5. Inputs

### UI Inputs
- Route: `/app/reconciliation` or story-specific nested route.
- Form fields: IDs/references required for Critical state-changing action; no raw source, secrets or full prompts.
...
```json
{
  ...
  "inputReference": "ref:06-04-prototype",
...
```json
{
  "status": "ACCEPTED",
  "assessmentId": "018f2b8c-7d80-7c5f-9c42-5892d11c2f21",
  "outputReference": "ref:06-04-output",
  "correlationId": "018f2b90-7d80-7c5f-9c42-5892d11c2f21"
}
```
- Error JSON:
...
### UI Output
- Success state: show Append-only AuditEvent with correlation ID.
...
3. Load predecessor state required by `STORY-01-02` and fail closed if absent or blocked.
...
| Missing predecessor `STORY-01-02` | 409/422 | Required previous step is incomplete | Blocked transition audit |
...
|---|---|---|
| Implements Immutable Audit Trail Foundation | API/Web/Worker files in plan | Unit + integration tests |
...
- Direct predecessor: `STORY-01-02`.
...
This story turns `STORY-06-04` into an executable controlled-prototype slice. The business outcome is: Append-only AuditEvent with correlation ID. It gives the developer exact ownership for package `@lcsp/audit`, API route `/api/v1/assessments/{assessmentId}/audit-events`, service `AuditService`, and persistence models `AuditEvent`.

### Before coding

- Required completed predecessor stories: STORY-01-02.
...
1. Create shared contract in `packages/contracts/src/story-06-04/` with DTO, response, error and event payload schemas.
2. Create Prisma model/migration for `AuditEvent` if the model does not already exist.
3. Create or update NestJS module files for `AuditController` and `AuditService` at the route `/api/v1/assessments/{assessmentId}/audit-events`.
...
10. Run manual smoke test and confirm observable output: Append-only AuditEvent with correlation ID.

### Concrete execution trace

User action: developer triggers the story through the Manager UI or an authenticated API request for `Critical state-changing action`.
→ API route: `/api/v1/assessments/{assessmentId}/audit-events`.
...
→ service: `AuditService` validates DTO and starts a Prisma transaction.
→ database transaction: create/update `AuditEvent` using UUIDv7 internal IDs and reference-only payloads.
...
→ worker: Node.js worker handler consumes `none` and calls package `@lcsp/audit`.
→ output state: API/UI exposes Append-only AuditEvent with correlation ID; blocked states include exact error code and do not expose raw source, prompt, token or secret.
...
- PostgreSQL contains expected `AuditEvent` rows.
...
- Manager UI shows the expected visible result: Append-only AuditEvent with correlation ID.
