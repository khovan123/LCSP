...
| Module | Range | Focus |
| --- | --- | --- |
| M01 - Authentication & Account Security | FR-001..FR-011, FR-063..FR-065, FR-074..FR-076 | Account, login, OAuth/OIDC, Authenticator App MFA, session, password reset, email verification, password/profile maintenance |
...
| M05 - Evidence Gates & Technical Profile | FR-031..FR-035, FR-069..FR-071 | Schema/privacy/quality gates, TechnicalProfile, AI Usage Flow Analysis, evidence status |
| M06 - Reconciliation & VerifiedProfile | FR-036..FR-042, FR-067, FR-072 | Conflict detection/routing/resolution, Manager final resolution, usage-flow reconciliation, VerifiedProfile approval |
| M07 - Risk Classification & Legal Rule Citation | FR-043..FR-047, FR-073 | Legal retrieval, usage-flow rule matching, classification, citation trace, blocked/degraded behavior |
...
| FR-025 | Connect GitHub repository | LCSP shall allow Manager to connect/select GitHub repository for own assessment, and allow Developer only when delegated. | Must | M04 | UC-M04-02 | Connect GitHub Repository | BR-032 | GitHub App uses read-only selected repo/scope; OAuth/OIDC login does not grant repository access. | None |
| FR-026 | Upload Local/CI scanner report | Deferred/Future/Enterprise: LCSP may later allow Developer to upload Local/CI evidence report. Not required for MVP backlog and not part of current main workflow. | Deferred / Future / Enterprise | M04 | UC-M04-03 | Upload Local/CI Scanner Report | BR-033 | If enabled later, uploaded report enters evidence schema gate. | None |
| FR-027 | Upload manual technical evidence JSON | Deferred/Future/Enterprise: LCSP may later allow Developer to upload structured manual technical evidence JSON. Not required for MVP backlog and not part of current main workflow. | Deferred / Future / Enterprise | M04 | UC-M04-04 | Upload Manual Technical Evidence JSON | BR-034 | If enabled later, free text cannot unlock classification. | A3 |
| FR-028 | Review technical findings | LCSP shall allow Manager to view technical findings for own assessment, and allow Developer only when delegated. | Must | M04 | UC-M04-05 | Review Technical Findings | BR-035 | Findings show evidence refs and confidence, not raw source snippets. | None |
| FR-029 | Respond to technical clarification | LCSP may allow delegated Developer to provide technical clarification within assigned scope, but Manager remains final conflict resolver. | Should | M04 | UC-M04-06 | Confirm Technical Truth | BR-046 | Developer input cannot finalize conflict or unlock classification without Manager resolution. | A3/Post-MVP |
...
| FR-034 | Generate TechnicalProfile | LCSP shall derive TechnicalProfile from accepted technical evidence. | Must | M05 | UC-M05-04 | Generate TechnicalProfile | BR-039 | TechnicalProfile is normalized and ready for AI Usage Flow Analysis. | None |
...
| FR-036 | Detect conflict | LCSP shall compare WizardProfile with TechnicalProfile and AIUsageFlow to decide whether any conflict exists. | Must | M06 | UC-M06-01 | Detect Conflict | BR-041 | `NO_CONFLICT_FOUND` builds VerifiedProfile; `CONFLICT_FOUND` pauses workflow. | A1/A3 |
| FR-037 | Calculate Conflict Score | LCSP may calculate Conflict Score as explanatory/supporting context for reconciliation conflicts. | Should | M06 | UC-M06-02 | Calculate Conflict Score | BR-048 | Conflict Score does not create severity-based workflow routes in MVP. | A1/A3 |
| FR-038 | Route conflict to Manager | LCSP shall route any MVP conflict to Manager conflict resolution task. | Must | M06 | UC-M06-03 | Route Conflict to Manager | BR-042 | Workflow is not blocked because Developer is absent. | A3 |
| FR-039 | Resolve technical conflict by Manager | LCSP shall allow Manager to resolve technical conflicts by reviewing evidence, requesting re-scan/correction, or using optional delegated clarification. | Must | M06 | UC-M06-04 | Resolve Technical Conflict | BR-042 | Manager resolution is required and sufficient for MVP conflict completion when evidence is adequate. | A3 |
...
| FR-041 | Optional delegated technical clarification | Post-MVP, LCSP may allow Manager to delegate scoped technical clarification to Developer without transferring final resolution authority. | Deferred / Future | M06 | UC-M06-06 | Post-MVP Delegated Technical Clarification | BR-044 | Developer clarification is audited and Manager remains final resolver. | A3/Post-MVP |
| FR-042 | Create VerifiedProfile | LCSP shall create VerifiedProfile only after gates pass and conflicts are resolved. | Must | M06 | UC-M06-07 | Create VerifiedProfile | BR-045 | VerifiedProfile is not created with any unresolved conflict. | A1/A3 |
| FR-043 | Retrieve legal rules/citations | LCSP shall retrieve versioned legal rules/citations for classification using VerifiedProfile and AIUsageFlow context. | Must | M07 | UC-M07-01 | Retrieve Legal Rules/Citations | BR-050 | Critical rules include rule_id, source, citation, version and effective date if available. | A2 |
...
| FR-066 | Run repository scan | LCSP shall allow Manager to run or re-run repository scan for own assessment, and allow Developer only when granted `RUN_REPOSITORY_SCAN` policy. | Must | M04 | UC-M04-08 | Run Repository Scan | BR-077 | Scan job is created only for valid repo/branch/commit and actor scope; output enters evidence gates. | None |
| FR-067 | Review and approve VerifiedProfile | LCSP shall require Manager review and approval of VerifiedProfile before `READY_FOR_CLASSIFICATION`. | Must | M06 | UC-M06-08 | Review and Approve VerifiedProfile | BR-078 | Manager cannot approve if any conflict or required confirmation is unresolved. | A1/A3 |
...
| FR-069 | Detect AI usage flow signals | LCSP shall extract AI input, output, decision-flow, automated-decision, human-review, user-impact, sensitive-data, domain-context, system-prompt-purpose and harm-potential signals from repository scan findings where available. | Must | M05 | UC-M05-04 | Generate TechnicalProfile | BR-080 | TechnicalEvidenceReport findings include usage-flow signal types or explicit unknowns. | A1/A2 |
| FR-070 | Generate AIUsageFlow | LCSP shall generate AIUsageFlow after TechnicalProfile and before Reconciliation. | Must | M05 | UC-M05-04 | Generate TechnicalProfile | BR-081 | AIUsageFlow captures purpose, input, output, downstream action, affected subject, human review, automation level, harm potential and evidence refs. | A1/A2 |
| FR-071 | Mark unclear AI usage purpose | LCSP shall mark AIUsageFlow fields as unknown/unclear when evidence is insufficient and avoid overclaiming. | Must | M05 | UC-M05-05 | Mark Evidence as Rejected, Insufficient, or Ready | BR-082 | Unclear usage flow creates confirmation need or conflict rather than unsupported classification. | A1/A2 |
| FR-072 | Reconcile AIUsageFlow | LCSP shall compare AIUsageFlow with WizardProfile and TechnicalProfile during reconciliation. | Must | M06 | UC-M06-01 | Detect Conflict | BR-083 | Usage-purpose conflicts pause workflow and keep classification locked. | A1/A3 |
| FR-073 | Match legal rules using AIUsageFlow | LCSP shall use AIUsageFlow fields for legal rule matching and must not classify risk based only on model/provider/framework presence. | Must | M07 | UC-M07-01 | Retrieve Legal Rules/Citations | BR-084 | Legal retrieval considers business process, AI purpose, automation level, affected subjects, data types, human review and harm categories. | A2 |
| FR-074 | Sign in with OAuth/OIDC | LCSP shall support OAuth/OIDC user login as an active MVP authentication capability. | Must | M01 | UC-M01-14 | Sign In with OAuth/OIDC | BR-086 | Valid provider callback creates LCSP session only after identity/session/MFA policy pass. | None |
| FR-075 | Validate OAuth/OIDC callback and account linking | LCSP shall validate OAuth/OIDC callback, `state`, `nonce`, issuer, audience, token expiry and safe account linking policy. | Must | M01 | UC-M01-14 | Sign In with OAuth/OIDC | BR-087 | Invalid callback/linking is rejected and audited; no raw provider token is exposed. | None |
| FR-076 | Separate OAuth/OIDC from GitHub repository access | LCSP shall keep OAuth/OIDC login separate from GitHub App repository connection and repository scan permission. | Must | M01/M04 | UC-M01-14 | Sign In with OAuth/OIDC | BR-088 | OAuth/OIDC login does not create repository connection or grant scan permission. | None |
| FR-077 | Manager superset MVP permissions | LCSP shall allow Manager to perform all active MVP assessment, evidence, conflict, classification, report and audit actions without Developer assignment. | Must | M02/M04/M06/M08 | UC-M03-01 | Create Assessment | BR-089 | No MVP workflow is blocked merely because Developer is absent. | None |
| FR-078 | Post-MVP delegated Developer permissions | LCSP shall model future Developer delegation as scoped, revocable grants that never reduce Manager permissions. | Deferred / Future | M02 | UC-M02-05 | Assign Developer Policy | BR-090 | Manager can revoke/reassign or perform delegated actions at any time. | A3/Post-MVP |
...
| --- | --- | --- |
| A1 - Wizard simplicity vs completeness | FR-019, FR-020, FR-021, FR-022, FR-023, FR-033, FR-035, FR-036, FR-042, FR-050, FR-067, FR-069..FR-072 | WizardProfile and reconciliation input may change after validation |
| A2 - Legal corpus / rule reliability | FR-043, FR-044, FR-045, FR-046, FR-047, FR-048, FR-049, FR-056, FR-068, FR-069..FR-071, FR-073 | Legal rule/citation contract, AIUsageFlow rule matching and classification behavior may change |
...
| Attestation | Which claims require Manager final review, optional delegated clarification or stricter post-validation controls remains validation-required. |
