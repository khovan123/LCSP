# Document Generation Developer Execution Blueprint

# Business Purpose

Generate compliance document shells and evidence appendix from classification, gap analysis, and citations.
...
Worker consumes `gap-analysis.completed.v1` after predecessor object is persisted.
...
```json
{
  ...
  "inputType": "GapAnalysisCompletedPayload",
...
{
  "assessmentId": "assess_001",
  "outputType": "GeneratedDocument",
  "status": "CREATED",
  "evidenceRefs": ["ev_001", "ev_002"],
  "nextEvent": "document.generated.v1 or document.blocked.v1"
}
```
...
|---:|---|---|---|---|---|---|
| 1 | Input received | `DocumentGenerationService.generate()` | Required predecessor records | None | Consumes `gap-analysis.completed.v1` or API trigger | Validated input DTO |
| 2 | Preconditions checked | `DocumentGenerationService.generate()` | `Assessment`, actor/state, source object | None | None | Guard pass or blocked error |
| 3 | Domain transform runs | `DocumentGenerationService.generate()` | Evidence/source rows | Draft output object | None | `GeneratedDocument` draft |
| 4 | Transaction commits | Repository layer | Existing object versions | `GeneratedDocument`, `AuditEvent`, `OutboxEvent` | staged `document.generated.v1 or document.blocked.v1` | Persisted `GeneratedDocument` |
| 5 | Event published | Outbox publisher | `OutboxEvent` | published marker | `document.generated.v1 or document.blocked.v1` | Next worker input |
| 6 | Next worker consumes | downstream worker | `GeneratedDocument` | downstream object or blocked state | next event | Workflow advances |

# Object Lifecycle

```text
ClassificationResult + GapAnalysis + CitationSet -> DocumentDraft -> GeneratedDocument
```
...
```text
Final legal conclusion is withheld or degraded when citation traceability is missing.
```
...
|---|---|---|---|
| Current predecessor | `lcsp.events` / `lcsp.commands` | `gap-analysis.completed.v1` | `DocumentWorker.handleGapAnalysisCompleted()` |
| `DocumentWorker.handleGapAnalysisCompleted()` | `lcsp.events` | `document.generated.v1 or document.blocked.v1` | Next workflow worker |
...
| Create | `GeneratedDocument`, `AuditEvent`, `OutboxEvent` |
...
sequenceDiagram
  ...
  participant Worker as DocumentWorker.handleGapAnalysisCompleted()
  participant DB as PostgreSQL
  participant Audit as Audit Log
  participant Next as Next Worker

  Prev->>MQ: publish gap-analysis.completed.v1
  MQ->>Worker: consume gap-analysis.completed.v1
  Worker->>DB: read predecessor + assessment state
  Worker->>DB: persist GeneratedDocument + outbox
  Worker->>Audit: write audit event
  Worker->>MQ: publish document.generated.v1 or document.blocked.v1
  MQ->>Next: deliver next event
```

# Developer Mental Model

Implement `Document Generation` as a deterministic object transformer. It receives one canonical input object, reads only the predecessor records it needs, creates exactly one canonical output object or a blocked state, writes an audit event, and emits the next event through outbox. Hidden synchronous jumps to later workflow stages are forbidden.

# Anti-Patterns

- Creating downstream objects before `GeneratedDocument` is persisted.
- Emitting `document.generated.v1 or document.blocked.v1` before DB commit.
...
1. Seed predecessor records for `GapAnalysisCompletedPayload`.
2. Insert or publish `gap-analysis.completed.v1` with correlation id `corr_assess_001`.
3. Run `DocumentWorker.handleGapAnalysisCompleted()` locally against fixture `F-RAG-02 Missing citation case`.
4. Verify `GeneratedDocument` row exists.
...
|---|---|
| `F-RAG-02 Missing citation case` | `GeneratedDocument` with expected status and evidence refs. |
