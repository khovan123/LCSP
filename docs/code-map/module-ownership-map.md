...
`packages/scanner` is no longer the lifecycle-owning scanner package. The active scanner runtime is the `scanner` module inside the `lcsp-python-workers` monorepo. A bounded Node.js TS/JS analyzer remains an adapter invoked by Python Worker.
