...
| `legal-ingestion-worker` | Node/TypeScript or bounded worker runtime | Fetch, hash, normalize, and stage legal sources. | `LegalSource`, `LegalDocument`, `LegalCorpusItem` | `lcsp.legal-ingestion-worker.v1` | `implementation/legal-corpus-ingestion-implementation.md` |
| `embedding-index-worker` | Node/TypeScript | Build pgvector embeddings and FTS indexes for approved corpus. | `LegalDocumentChunk`, `ModelRunMetadata` | `lcsp.embedding-index-worker.v1` | `implementation/hybrid-retriever-implementation.md` |
