...
| `classification` | Classification status/results endpoints. | verified profile, legal matching, audit. | `RiskClassificationDto`, `ClassificationBlockedDto` | `RiskClassification`, `LegalRuleMatch`, `AuditEvent` | None from API; worker emits `event.classification.completed.v1` or `event.classification.blocked.v1` | `/api/v1/assessments/:assessmentId/classifications/*` | `specs/legal-classification-spec.md` |
| `gap-analysis` | Gap-analysis result access and document precondition checks. | classification, legal matching, audit. | `GapAnalysisDto`, `GapAnalysisBlockedDto` | `GapAnalysis`, `AuditEvent` | worker emits `event.gap-analysis.completed.v1`, `event.gap-analysis.blocked.v1`, or `event.gap-analysis.failed.v1` | `/api/v1/assessments/:assessmentId/gap-analysis/latest` | `specs/legal-classification-spec.md` |
| `documents` | Document request/status/download endpoints. | gap analysis, classification, citation traceability, object storage, audit. | `DocumentRequestDto`, `GeneratedDocumentDto` | `GeneratedDocument`, `AuditEvent` | `command.document.requested.v1` | `/api/v1/assessments/:assessmentId/documents/*` | `specs/document-generation-spec.md` |
...
| `gap-analysis-worker` | Derive compliance gaps from completed classification and legal basis. | classification, legal matching. | `GapAnalysisRequestedPayload`, `GapAnalysisCompletedPayload`, `GapAnalysisBlockedPayload` | `GapAnalysis` | `lcsp.gap-analysis-worker.v1` | None | `specs/legal-classification-spec.md` |
