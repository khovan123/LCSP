...
| Repository Deep Agent                | Owns repository evidence derivation inside a durable LangGraph thread and LCSP Docker sandbox. Uses native Deep Agents filesystem/shell/subagents over the commit-pinned repository and optional Codebase Memory MCP structural memory; direct source remains authoritative. | Queue boundary, Persistence, Repository Integration, Docker sandbox, Codebase Memory MCP. |
...
- Repository analysis is source-grounded agent analysis inside the LCSP Docker sandbox, not a fixed static-scanner pipeline.
...
- Repository-analysis failure, coverage, provenance, and retry policy is defined by the active Agent Runtime and repository-analysis architecture; the archived scanner severity decision is historical only.
