...
|---|---|---|
| `flow_auth_signin` | **Auth & Sign-in** | `Sign-in Page` (`sign-in/page.tsx`) ➔ `SignInForm Organism` ➔ `apiRequest()` ➔ `BFF Route` (`api/auth/sign-in/route.ts`) ➔ `upstreamRequest` ➔ `AuthWorkspaceController` ➔ `PbacEvaluatorService` ➔ `Prisma Service` ➔ `AuthAuditEvent` ➔ Session Cookie |
...
| `erd_auth_domain` | **ERD - Auth & PBAC Domain** | `AuthOrganization`, `AuthUser`, `AuthPolicy`, `AuthMembership`, `AuthSession`, `AuthAuditEvent`. |
...
|---|---|---|---|
| `uc_actor_compliance_manager` | **Compliance Manager** | **Compliance Manager Master Journey** | Sign-in ➔ Org PBAC ➔ Create Assessment ➔ Wizard Declarations ➔ Connect Repo ➔ Resolve Conflicts ➔ Export Audit & PDF Report |
...
| `uc_actor_developer` | **Developer** | **Developer Master Journey** | Authenticate ➔ Validate Scoped PBAC ➔ Grant Repository Read Access |
...
|---|---|---|
| `uc001_authenticate` | **UC-001 Authenticate and Manage Account** | Manager/Developer ➔ Web ➔ API ➔ DB ➔ Audit |
| `uc002_manage_pbac` | **UC-002 Manage Organization and PBAC Scope** | Manager ➔ Web ➔ API ➔ DB ➔ Contracts |
