...
import type { NormalizedWorkflowStep } from "@/features/workspace/types/assessment-runtime-adapter.types";
  ...
  return (
    <section aria-label={resolveAppMessage("pages.appShell.runtimePanelTitle")}>
      <p className="mb-2 text-[0.6875rem] font-semibold tracking-widest text-muted-foreground uppercase">
        {resolveAppMessage("pages.appShell.runtimePanelTitle")}
      </p>
      <ol className="rounded-xl border border-border/70 bg-card px-3.5">
        {steps.map((step) => (
          <WorkflowStatusRow key={step.id} step={step} />
        ))}
      </ol>
    </section>
  );
