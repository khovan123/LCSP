...
export function ArtifactEvidenceRow({ item, onOpenArtifact }: { item: NormalizedAssessmentArtifactItem; onOpenArtifact?: (ref: NormalizedAssessmentArtifactItem["ref"]) => void }) {
  ...
  if (item.ref.type === ARTIFACT_TYPES.programEvidenceGraph && onOpenArtifact) return <button type="button" onClick={() => onOpenArtifact(item.ref)} className="flex w-full min-w-0 items-center gap-2.5 rounded-lg border border-border/60 px-2.5 py-2.5 text-left transition-colors hover:bg-muted/50" aria-label={`${resolveAppMessage("pages.artifacts.open" as MessageKey)} ${label}`}>{content}</button>;
