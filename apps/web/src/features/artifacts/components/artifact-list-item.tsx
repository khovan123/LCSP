...
export function ArtifactListItem({ item }: { item: ArtifactListItemModel }) {
  ...
  if (
    ...
    return (
      <Link
        href={target.href}
        className="flex min-w-0 items-center gap-3 rounded-xl border p-3 transition-colors hover:bg-muted/50"
        aria-label={`${resolveAppMessage("pages.artifacts.open")} ${item.title}`}
      >
        {content}
      </Link>
    );
  ...
  return (
    <div className="flex min-w-0 items-center gap-3 rounded-xl border p-3 opacity-70">
      {content}
    </div>
  );
}
