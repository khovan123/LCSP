...
describe("OutboxDlqService", () => {
  ...
  describe("replayMessage", () => {
    it("should replay a valid DLQ message and write audit", async () => {
      const mockMessage = { id: "1", status: "dlq", eventType: "TEST", aggregateId: "123" } as OutboxMessageEntity;
    ...
    it("should throw NotFoundException if message is not in DLQ", async () => {
      ...
      await expect(service.replayMessage("1", "actor-1")).rejects.toThrow(NotFoundException);
  ...
  describe("deleteMessage", () => {
    ...
    it("should throw NotFoundException if message is not in DLQ", async () => {
      const mockMessage = { id: "1", status: "published" } as OutboxMessageEntity;
      outboxRepository.findMessageById.mockResolvedValue(mockMessage);

      await expect(service.deleteMessage("1", "actor-1")).rejects.toThrow(NotFoundException);
