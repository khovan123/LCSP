...
describe("DocumentController PBAC", () => {
  it("requires the document:generate PBAC action for final report", () => {
    ...
    expect(metadata).toEqual({ type: "action", action: PBAC_ACTIONS.documentGenerate });
  });
...
describe("DocumentController dispatch", () => {
  it("dispatches RequestGapAnalysisCommand", async () => {
    ...
    const req = { pbacContext: { organizationId: "org-1", userId: "user-1" }, correlationId: "corr-1" } as any;

    await controller.requestGapAnalysis("asmt-1", req);

    expect(execute).toHaveBeenCalledTimes(1);
    expect(execute.mock.calls[0]?.[0]).toBeInstanceOf(RequestGapAnalysisCommand);
  });
});
