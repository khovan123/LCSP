...
import { AssessmentNotFoundException, WizardAlreadySubmittedException } from '../../../domain/exceptions/wizard.exceptions.js';
  ...
  describe('execute', () => {
    ...
    it('T01: should create a new draft if none exists and log audit event', async () => {
      ...
      wizardRepository.upsertDraft.mockImplementation((profile) => {
        ...
        return Promise.resolve(new WizardProfileEntity({
          ...profile,
          id: 'wizard-id-1',
          createdAt: new Date(),
          updatedAt: new Date(),
      ...
      expect(wizardRepository.verifyAssessmentOwnership).toHaveBeenCalledWith('assessment-123', 'org-1', 'owner-1');
      expect(wizardRepository.findByAssessmentId).toHaveBeenCalledWith('assessment-123');
    ...
    it('T02: should update an existing IN_PROGRESS draft and bump version', async () => {
      wizardRepository.verifyAssessmentOwnership.mockResolvedValue(true);
      wizardRepository.findByAssessmentId.mockResolvedValue(new WizardProfileEntity({
        id: 'wizard-id-2',
        assessmentId: 'assessment-123',
        organizationId: 'org-1',
        ownerId: 'owner-1',
      ...
      wizardRepository.upsertDraft.mockImplementation((p) => Promise.resolve(new WizardProfileEntity({
        ...p,
        updatedAt: new Date(),
      })));
    ...
    it('T03: should throw WizardAlreadySubmittedException if status is SUBMITTED', async () => {
      ...
      await expect(handler.execute(command)).rejects.toThrow(WizardAlreadySubmittedException);
    ...
    it('T04: should throw AssessmentNotFoundException if ownership verification fails', async () => {
      ...
      await expect(handler.execute(command)).rejects.toThrow(AssessmentNotFoundException);
