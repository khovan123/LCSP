/**
 * Evidence Graph Contracts & Types
 *
 * Defines all TypeScript interfaces for evidence graph visualization:
 * - Node/edge/cluster domain models
 * - API response envelopes
 * - Graph-specific query parameters
 */
...
/**
 * Request to fetch evidence graph.
 * Used internally by query handler.
 */
export interface GetEvidenceGraphRequest {
  assessmentId: string;
  organizationId: string;
  userId: string;
  subjectRole: "MANAGER" | "DEVELOPER";
  scope: GraphScope;
  clusterId?: string; // Required when scope=detail
  correlationId: string;
}

/**
 * Response problem details (error).
 */
