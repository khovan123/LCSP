...
import { getAdminUserDetail } from "./admin-account.reader.js";
import {
  accountTransaction,
  assertCurrentAdmin,
  receipt,
...
export async function mutateAdminAccount(
  ...
  const input = record(raw, actor.correlationId, ["expectedVersion", "reason"]);
  ...
  return accountTransaction(prisma, actor.correlationId, async (tx) => {
    ...
    return {
      ...(await getAdminUserDetail(tx, id, actor.correlationId)),
      replayed: false,
    };
  });
}
