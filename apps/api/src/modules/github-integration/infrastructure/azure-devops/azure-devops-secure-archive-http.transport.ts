...
const ALLOWED_HOSTS = new Set(["dev.azure.com", "visualstudio.com"]);
...
export class AzureDevOpsSecureArchiveHttpTransport implements GitHubArchiveTransportPort {
  ...
  async downloadArchive(input: {
    ...
    if (
      !REPOSITORY_PATH_PATTERN.test(input.repositoryFullName) ||
    ...
    try {
      ...
      let url = `https://dev.azure.com/${encodeURIComponent(org)}/${encodeURIComponent(project)}/_apis/git/repositories/${encodeURIComponent(repo)}/items?scopePath=/&recursionLevel=full&includeContent=true&versionDescriptor.version=${input.commitSha}&versionDescriptor.versionType=commit&$format=zip&api-version=7.1-preview`;
      ...
      for (let redirects = 0; ; redirects += 1) {
        ...
        if (
          next.protocol !== "https:" ||
          (!ALLOWED_HOSTS.has(next.hostname) &&
            !next.hostname.endsWith(".visualstudio.com"))
        ) {
          throw new GitHubArchiveTransportError(
            GITHUB_CREDENTIAL_ERROR_CODES.providerResponseInvalid,
          );
