  ...
  implements GitHubArchiveTransportPort
{
  ...
  async downloadArchive(input: {
    ...
    if (
      !REPOSITORY_PATTERN.test(input.repositoryFullName) ||
    ...
    try {
      const [workspace, repo] = input.repositoryFullName.split("/");
      let url = `https://bitbucket.org/${encodeURIComponent(workspace)}/${encodeURIComponent(repo)}/get/${input.commitSha}.tar.gz`;
