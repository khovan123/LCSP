...
export class GetSystemGraphHandler implements IQueryHandler<
  GetSystemGraphQuery,
  SystemGraphDto
> {
  ...
  async execute(query: GetSystemGraphQuery): Promise<SystemGraphDto> {
    ...
    const reconciliations =
      await this.prisma.graphReconciliationResult.findMany({
        where: { assessmentId },
      });
    ...
    for (const r of reconciliations) {
      ...
      if (r.declaredNodeId) {
        reconciliationMap.set(r.declaredNodeId, {
          status,
          partnerId: r.observedNodeId || undefined,
        });
      }
      if (r.observedNodeId) {
        reconciliationMap.set(r.observedNodeId, {
          status,
          partnerId: r.declaredNodeId || undefined,
        });
      }
    }
