...
const SECRET_PATTERNS = /ghp_[A-Za-z0-9]+|sk-ant-[A-Za-z0-9]+|AKIA[A-Z0-9]+|Bearer\s+[A-Za-z0-9._-]{20,}/;
const PLAINTEXT_SECRET = /("password"\s*:\s*"[^"]{1,}"|"mfa_secret"\s*:|"plaintext_token"\s*:)/;
...
describe("Redaction boundary (e2e) [AC-022]", () => {
  ...
  it("AC-022: Sign-in response does not include plaintext password", async () => {
    ...
    assert.doesNotMatch(body, /CorrectHorseBatteryStaple/, "Response must not echo back password");
    assert.doesNotMatch(body, PLAINTEXT_SECRET, "Response must not include plaintext password field");
  });
  ...
  it("AC-022: User profile response does not include password hash", async () => {
    ...
    assert.doesNotMatch(body, /passwordHash|password_hash/, "Profile must not expose password hash");
    assert.doesNotMatch(body, /mfaSecret|mfa_secret/, "Profile must not expose MFA secret");
  });
  ...
  it("AC-022: Assessment response does not include any finding source code", async () => {
    ...
    assert.doesNotMatch(body, /def [a-z_]+\(/, "Response must not include Python source code");
    assert.doesNotMatch(body, /function [a-z]+\(/, "Response must not include JS source code");
    assert.doesNotMatch(body, /import [A-Za-z]+\s+from/, "Response must not include import statements");
  });
